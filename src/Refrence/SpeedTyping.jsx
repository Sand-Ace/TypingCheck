...
const SpeedTypingGame = () => {
  ...
  // Update the useEffect for the reset
  useEffect(() => {
    loadParagraph();
  }, []);
    ...
    if (isTyping && timeLeft > 0) {
      interval = setInterval(() => {
        setTimeLeft((prevTime) => {
          ...
          let cpm = (charIndex - mistakes) * (60 / (maxTime - newTime));
          cpm = cpm < 0 || !cpm || cpm === Infinity ? 0 : cpm;
          setCPM(parseInt(cpm, 10));
          let wpm = Math.round(
            ((charIndex - mistakes) / 5 / (maxTime - newTime)) * 60
          );
          wpm = wpm < 0 || !wpm || wpm === Infinity ? 0 : wpm;
          setWPM(wpm);
          return newTime;
    ...
    <>
      <div className="container">
        ...
        <TypingArea
          ...
          resetGame={resetGame}
        />
        {timeLeft === 0 && typingText.length !== inpFieldValue.length ? (
          <p>Time is up!!!</p>
        ) : undefined}
      </div>
    </>
